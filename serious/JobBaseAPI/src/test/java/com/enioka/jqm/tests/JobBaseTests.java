...
public class JobBaseTests {
	...
	public void testInit() {
		...
		this.jdDemoMaven = CreationTools.createJobDefinition(true, "DemoMavenClassName", "/Users/pico/Dropbox/projets/enioka/tests/DateTimeMaven/",
		        qNormal, 42, "MarsuApplication", 42, "Franquin", "ModuleMachin", "other", "other", "other", true);
	...
	public void testChangeQueueIntVersion() {
		...
		JobInstance j = CreationTools.em.createQuery("SELECT j FROM JobInstance j WHERE j.jd.id = :job", JobInstance.class)
		        .setParameter("job", jd.getId()).getSingleResult();

		Dispatcher.changeQueue(j.getId(), qSlow.getId());

		j = CreationTools.emf.createEntityManager().createQuery("SELECT j FROM JobInstance j WHERE j.jd.id = :job", JobInstance.class)
		        .setParameter("job", jd.getId()).getSingleResult();
	...
	public void testDelJobInQueue() {
		...
		JobInstance q = CreationTools.em.createQuery("SELECT j FROM JobInstance j, JobDefinition jd WHERE j.jd.id = :job", JobInstance.class)
		        .setParameter("job", jd.getId()).getSingleResult();

		Dispatcher.delJobInQueue(q.getId());

		Query tmp = CreationTools.em.createQuery("SELECT j FROM JobInstance j, JobDefinition jd WHERE j.jd.id = :job", JobInstance.class)
	...
	public void testCancelJobInQueue() {
		...
		JobInstance q = CreationTools.emf.createEntityManager()
		        .createQuery("SELECT j FROM JobInstance j, JobDefinition jd WHERE j.jd.id = :job", JobInstance.class).setParameter("job", jd.getId())
		        .getSingleResult();
	...
	public void testGetUserJobs() {
		...
		ArrayList<JobInstance> tmp = (ArrayList<JobInstance>) CreationTools.em
		        .createQuery("SELECT j FROM JobInstance j WHERE j.user = :u", JobInstance.class).setParameter("u", "MAG").getResultList();
	...
	public void testGetJobs() {
		...
		ArrayList<JobInstance> tmp = (ArrayList<JobInstance>) CreationTools.em.createQuery("SELECT j FROM JobInstance j", JobInstance.class)
		        .getResultList();
	...
	public void testGetQueues() {
		...
		ArrayList<Queue> tmp = (ArrayList<Queue>) CreationTools.em.createQuery("SELECT j FROM Queue j", Queue.class).getResultList();
