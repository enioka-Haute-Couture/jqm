...
foreach (${Target} in ${targets}.targets.target) {
    ...
    if ($Compose) {
        ...
        if (-not $SkipSubImages) {
            foreach ($preBuild in @($Target.subTargets.target)) {
                ...
                if ($PSCmdlet.ShouldProcess(${service}, 'Compose Build')) {
                    docker compose -f $Compose build @buildArgsPre ${service} >>$LogFile
                    if (-not $?) {
                        throw "Build error"
                    }
                }
        ...
        if ($PSCmdlet.ShouldProcess("JQM", 'Compose Build')) {
            ...
            docker compose -f $Compose build @buildArgs jqm >>$LogFile
            if (-not $?) {
                throw "Build error"
            }
        }
        ...
        if ($Push) {
            ...
            if ($PSCmdlet.ShouldProcess("JQM", 'Compose Push')) {
                docker compose -f $Compose push jqm >>$LogFile
                if (-not $?) {
                    throw "Push error"
                }
            }
