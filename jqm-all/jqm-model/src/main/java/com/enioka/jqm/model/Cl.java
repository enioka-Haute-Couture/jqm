package com.enioka.jqm.model;

import java.io.Serializable;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;

import com.enioka.jqm.jdbc.DatabaseException;
import com.enioka.jqm.jdbc.DbConn;
import com.enioka.jqm.jdbc.NoResultException;
import com.enioka.jqm.jdbc.QueryResult;

/**
 * <strong>Not part of any API - this an internal JQM class and may change without notice.</strong> <br>
 * Persistence class for storing the definition of the class loaders used by the {@link JobDef}.
 */
public class Cl implements Serializable
{
    private static final long serialVersionUID = -965329631668297388L;

    private long id;

    private String name;

    private boolean childFirst = false;

    private String hiddenClasses;

    private boolean tracingEnabled = false;

    private boolean persistent = false;

    private String allowedRunners;

    private String extraClasspathDirs;

    private String excludedClassPrefixes;

    /**
     * A technical ID without any meaning. Generated by the database.
     *
     * @return the unique identifier of this class loader
     */
    public long getId()
    {
        return id;
    }

    /**
     * See {@link #setId(long)}
     *
     * @param id
     *            the unique identifier to set
     */
    public void setId(long id)
    {
        this.id = id;
    }

    /**
     * The key used to identify the class loader in the deployment descriptor. Unique.
     *
     * @return the name of this class loader
     */
    public String getName()
    {
        return name;
    }

    /**
     * See {@link #getName()}
     *
     * @param name
     *            the name to set for this class loader
     */
    public void setName(String name)
    {
        this.name = name;
    }

    /**
     * Offer option to have child first class loading. Default should stay parent first, as it is the norm in JSE.
     *
     * @return true if child-first class loading is enabled
     */
    public boolean isChildFirst()
    {
        return childFirst;
    }

    /**
     * See {@link #isChildFirst()}
     *
     * @param childFirst
     *            true to enable child-first class loading
     */
    public void setChildFirst(boolean childFirst)
    {
        this.childFirst = childFirst;
    }

    /**
     * Offer possibility to hide Java classes from jobs. One or more regex defining classes never to load from the parent class loader.
     *
     * @return the regex pattern for hidden classes
     */
    public String getHiddenClasses()
    {
        return hiddenClasses;
    }

    /**
     * See {@link #getHiddenClasses()}
     *
     * @param hiddenClasses
     *            the regex pattern for classes to hide
     */
    public void setHiddenClasses(String hiddenClasses)
    {
        this.hiddenClasses = hiddenClasses;
    }

    /**
     * Activate listing all class loaded inside the job log
     *
     * @return true if class loading tracing is enabled
     */
    public boolean isTracingEnabled()
    {
        return tracingEnabled;
    }

    /**
     * See {@link #isTracingEnabled()}
     *
     * @param trace
     *            true to enable class loading tracing
     */
    public void setTracingEnabled(boolean trace)
    {
        this.tracingEnabled = trace;
    }

    /**
     * The different event handlers associated to this class loader. Ordered list.
     *
     * @return list of handlers for this class loader
     */
    public List<ClHandler> getHandlers()
    {
        return handlerCache;
    }

    private List<ClHandler> getHandlers(DbConn cnx)
    {
        return ClHandler.select(cnx, "cleh_select_all_for_cl", this.id);
    }

    /**
     * Default is false. When false, the class loader is transient: it is created to run a new job instance and is thrown out when the job
     * instance ends. When true, it is not thrown out at the end and will be reused by all job instances created from the different
     * {@link JobDef}s using this {@link Cl} (therefore, multiple {@link JobDef}s can share the same static context).
     *
     * @return true if this class loader should be persistent
     */
    public boolean isPersistent()
    {
        return persistent;
    }

    /**
     * See {@link #isPersistent()}
     *
     * @param persistent
     *            true to make this class loader persistent
     */
    public void setPersistent(boolean persistent)
    {
        this.persistent = persistent;
    }

    /**
     * The different runners that are active in this context. If null, the global parameter job_runners is used instead.
     *
     * @return the allowed runners configuration
     */
    public String getAllowedRunners()
    {
        return allowedRunners;
    }

    /**
     * See {@link #getAllowedRunners()}
     *
     * @param allowedRunners
     *            the allowed runners configuration to set
     */
    public void setAllowedRunners(String allowedRunners)
    {
        this.allowedRunners = allowedRunners;
    }

    /**
     * A comma-separated list of directory paths to add to the classpath of the class loader.
     *
     * @return the extra classpath directories
     */
    public String getExtraClasspathDirs()
    {
        return extraClasspathDirs;
    }

    /**
     * See {@link #getExtraClasspathDirs()}
     *
     * @param extraClasspathDirs
     *            the comma-separated directory paths to add to the class loader
     */
    public void setExtraClasspathDirs(String extraClasspathDirs)
    {
        this.extraClasspathDirs = extraClasspathDirs;
    }

    /**
     * A comma-separated list of class prefixes that will be excluded from the class loader.
     * @return the excluded class prefixes
     */
    public String getExcludedClassPrefixes()
    {
        return excludedClassPrefixes;
    }

    /**
     * See {@link #getExcludedClassPrefixes()}
     *
     * @param excludedClassPrefixes the comma-separated class prefixes to exclude from the class loader
     */
    public void setExcludedClassPrefixes(String excludedClassPrefixes)
    {
        this.excludedClassPrefixes = excludedClassPrefixes;
    }

    /**
     * ResultSet is not modified (no rs.next called).
     *
     * @param rs
     *            the ResultSet containing the class loader data
     * @param colShift
     *            column offset for reading data from the ResultSet
     * @return a new Cl instance mapped from the ResultSet
     */
    static Cl map(ResultSet rs, int colShift)
    {
        Cl tmp = new Cl();

        try
        {
            tmp.id = rs.getLong(1 + colShift);
            tmp.name = rs.getString(2 + colShift);
            tmp.childFirst = rs.getBoolean(3 + colShift);
            tmp.hiddenClasses = rs.getString(4 + colShift);
            tmp.tracingEnabled = rs.getBoolean(5 + colShift);
            tmp.persistent = rs.getBoolean(6 + colShift);
            tmp.allowedRunners = rs.getString(7 + colShift);
            tmp.extraClasspathDirs = rs.getString(8 + colShift);
            tmp.excludedClassPrefixes = rs.getString(9 + colShift);
        }
        catch (SQLException e)
        {
            throw new DatabaseException(e);
        }
        return tmp;
    }

    private List<ClHandler> handlerCache = new ArrayList<>();

    public static List<Cl> select(DbConn cnx, String queryKey, Object... args)
    {
        List<Cl> res = new ArrayList<>();
        try (ResultSet rs = cnx.runSelect(queryKey, args))
        {
            while (rs.next())
            {
                Cl tmp = map(rs, 0);
                res.add(tmp);

                tmp.handlerCache = tmp.getHandlers(cnx);
            }
        }
        catch (SQLException e)
        {
            throw new DatabaseException(e);
        }
        return res;
    }

    public static Cl selectKey(DbConn cnx, String name)
    {
        List<Cl> res = select(cnx, "cl_select_by_key", name);
        if (res.isEmpty())
        {
            throw new NoResultException("no result for query by key for key " + name);
        }
        if (res.size() > 1)
        {
            throw new DatabaseException("Inconsistent database! Multiple results for query by key for key " + name);
        }
        return res.get(0);
    }

    public static Long create(DbConn cnx, String name, boolean childFirst, String hiddenClasses, boolean tracing, boolean persistent,
            String allowedRunners)
    {
        return create(cnx, name, childFirst, hiddenClasses, tracing, persistent, allowedRunners, null, null);
    }

    public static Long create(DbConn cnx, String name, boolean childFirst, String hiddenClasses, boolean tracing, boolean persistent,
            String allowedRunners, String extraClasspathDirs, String excludedClassPrefixes)
    {
        QueryResult r = cnx.runUpdate("cl_insert", name, childFirst, hiddenClasses, tracing, persistent, allowedRunners,
                extraClasspathDirs, excludedClassPrefixes);
        Long newId = r.getGeneratedId();

        return newId;
    }

    public void update(DbConn cnx)
    {
        if (id == 0)
        {
            this.id = Cl.create(cnx, name, childFirst, hiddenClasses, tracingEnabled, persistent, allowedRunners,
                extraClasspathDirs, excludedClassPrefixes);
        }
        else
        {
            cnx.runUpdate("cl_update_all_fields_by_id", name, childFirst, hiddenClasses, tracingEnabled, persistent, allowedRunners,
                    extraClasspathDirs, excludedClassPrefixes, id);
        }
    }
}
