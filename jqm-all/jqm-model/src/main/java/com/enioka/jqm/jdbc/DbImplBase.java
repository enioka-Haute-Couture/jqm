...
public class DbImplBase
{
    ...
    static {
        ...
        queries.put("jndi_update_changed_by_id", "UPDATE JNDIOBJECTRESOURCE SET AUTH=?, DESCRIPTION=?, FACTORY=?, LASTMODIFIED=CURRENT_TIMESTAMP, NAME=?, SINGLETON=?, TEMPLATE=?, TYPE=? WHERE ID=? AND NOT (AUTH=? AND DESCRIPTION=? AND FACTORY=? AND NAME=? AND SINGLETON=? AND TEMPLATE=? AND TYPE=?)");
        ...
        queries.put("jndiprm_select_all_in_jndisrc_list", "SELECT ID, KEYNAME, LASTMODIFIED, VALUE, RESOURCE_ID FROM JNDIOBJECTRESOURCEPARAMETER WHERE RESOURCE_ID IN (UNNEST(?))");
