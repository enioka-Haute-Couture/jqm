...
class DbImplBase
{
    ...
    static
    {
        ...
        // CL
        queries.put("cl_insert", "INSERT INTO __T__CL(ID, NAME, CHILD_FIRST, HIDDEN_CLASSES, TRACING, PERSISTENT, ALLOWED_RUNNERS, EXTRA_CLASSPATH_DIRS, EXCLUDED_CLASS_PREFIXES) VALUES(JQM_PK.nextval, ?, ?, ?, ?, ?, ?, ?, ?)");
        queries.put("cl_delete_all", "DELETE FROM __T__CL");
        queries.put("cl_delete_by_id", "DELETE FROM __T__CL WHERE ID=?");
        queries.put("cl_update_all_fields_by_id", "UPDATE __T__CL SET NAME=?, CHILD_FIRST=?, HIDDEN_CLASSES=?, TRACING=?, PERSISTENT=?, ALLOWED_RUNNERS=?, EXTRA_CLASSPATH_DIRS=?, EXCLUDED_CLASS_PREFIXES=? WHERE ID=?");
        queries.put("cl_select_all", "SELECT ID, NAME, CHILD_FIRST, HIDDEN_CLASSES, TRACING, PERSISTENT, ALLOWED_RUNNERS, EXTRA_CLASSPATH_DIRS, EXCLUDED_CLASS_PREFIXES FROM __T__CL ");
        ...
        // WITNESS
        queries.put("read_witness_node_id", "SELECT NODE FROM WITNESS WHERE KEYNAME='SCHEDULER'");
