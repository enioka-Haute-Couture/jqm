...
public class DbImplBase
{
    ...
    static {
        ...
        queries.put("jndi_update_changed_by_id", "UPDATE JNDIOBJECTRESOURCE SET AUTH=?, DESCRIPTION=?, FACTORY=?, LASTMODIFIED=CURRENT_TIMETAMP, NAME=?, SINGLETON=?, TEMPLATE=?, TYPE=? WHERE ID=? AND NOT (AUTH=? AND DESCRIPTION=? AND FACTORY=? AND NAME=? AND SINGLETON=? AND TEMPLATE=? AND TYPE=?)");
