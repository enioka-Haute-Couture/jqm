...
Deploying JQM behind a reverse proxy with a path prefix (e.g. https://myserver/jqm) is supported.

Set the JQM_BASE_PATH environment variable to the desired prefix (e.g. /jqm) before starting JQM. The value must have a leading slash and no trailing slash.

On startup, the <base> tag in index.html will be patched to the value of JQM_BASE_PATH, allowing the UI to work properly behind the reverse proxy.
