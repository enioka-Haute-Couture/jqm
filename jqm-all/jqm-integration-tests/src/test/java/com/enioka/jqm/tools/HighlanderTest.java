...
public class HighlanderTest extends JqmBaseTest
{
    ...
    public void testHighlanderMultiNode() throws Exception
    {
        ...
        JobRequest j1 = new JobRequest("MarsuApplication1", "TestUser");
        JobRequest j2 = new JobRequest("MarsuApplication2", "TestUser");
        for (int i = 0; i < 9; i++)
        {
            JqmClientFactory.getClient().enqueue(j1);
        }
        ...
        {
            JqmClientFactory.getClient().enqueue(j1);
            JqmClientFactory.getClient().enqueue(j2);
        }
        ...
        List<com.enioka.jqm.api.JobInstance> res = Query.create().addSortAsc(Sort.ID).setApplicationName("MarsuApplication1").run();
