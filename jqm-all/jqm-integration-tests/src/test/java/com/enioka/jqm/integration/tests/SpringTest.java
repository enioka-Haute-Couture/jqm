...
public class SpringTest extends JqmBaseTest
{
    ...
    /**
     * Verifies that the extraClasspathDirs ClassLoader attributes adds the given directory to the class loader, making resources placed
     * there visible to the running job.
     */
    @Test
    public void testClassLoaderWithExtraClasspathDirs() throws Exception
    {
        // Create a temporary directory that will be added to the class loader and place a test file in it
        File extraDir = new File("./target/extra-classpath-test-dir");
        FileUtils.deleteQuietly(extraDir);
        extraDir.mkdirs();
