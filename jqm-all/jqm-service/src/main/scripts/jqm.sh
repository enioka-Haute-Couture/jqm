...
JAVA_OPTS=${JAVA_OPTS:--Xms128m -Xmx512m -XX:MaxMetaspaceSize=128m}
...
jqm_createnode() {
    $JAVA -jar $JQM_JAR Update-Schema
    $JAVA -jar $JQM_JAR New-Node -n $JQM_NODE
}
...
jqm_import_xml() {
    $JAVA -jar $JQM_JAR Import-JobDef -f $1
}

jqm_import_queues() {
    $JAVA -jar $JQM_JAR Import-Queue -f $1
}

jqm_import_all_xml() {
    $JAVA -jar $JQM_JAR Import-JobDef -f $(find jobs -name "*xml" -type f | grep -v pom.xml | tr "\\n" " ")
}
...
jqm_enable_gui() {
    $JAVA -jar $JQM_JAR Set-WebConfiguration -c ENABLE_HTTP_GUI
    $JAVA -jar $JQM_JAR Reset-Root -p "$1"
}
