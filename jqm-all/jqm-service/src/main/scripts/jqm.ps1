<#
    ...
    The different possible actions are:
       ...
       * getrole: display a list of user roles
...
param(
    ...
    [ValidateSet("start", "allxml", "startconsole", "createnode", "stop", "status", "installservice", "removeservice", "enqueue", "resetpassword", "enablegui", "getrole")]
...
if (-not $PSVersionTable -or $PSVersionTable.PSVersion.Major -lt 4) {
    throw "This script is for PowerShell 4 and later only"
    return
}
...
function New-JqmService {
    if (-not (Get-Service $ServiceName -ErrorAction SilentlyContinue)) {
        ...
        Register-JqmNode
        & .\bin\jqmservice-64.exe //IS//$ServiceName --Description="Job Queue Manager node $NodeName" --JavaHome="$JAVA_HOME" --Jvm="$(${dll}.FullName)" --Startup="auto" --ServiceUser="$ServiceUser" --ServicePassword="$ServicePassword" --StartMode="jvm" --StopMode="jvm" --StartPath=$PSScriptRoot ++StartParams="Start-Node;-n;$NodeName" --StartClass="com.enioka.jqm.tools.Main" --StopClass="com.enioka.jqm.tools.Main" --StartMethod="start" --StopMethod="stop" --LogPath="$PSScriptRoot\logs" --StdOutput="auto" --StdError="auto" --Classpath="$PSScriptRoot\jqm.jar" ++JvmOptions="$($env:JAVA_OPTS.Replace(' ', ';'));$JAVA_OOM_ACTION"
        if (!$?) {
...
function Register-JqmNode {
    ...
    & $java -jar jqm.jar New-Node -n $NodeName
}

function Submit-JqmRequest([ValidateNotNullOrEmpty()]$JobDef) {
    Set-Location $PSScriptRoot
    & $java -jar jqm.jar New-Ji --application $JobDef
}

function Reset-RootPassword([ValidateNotNullOrEmpty()]$RootPassword) {
    Set-Location $PSScriptRoot
    & $java -jar jqm.jar "Reset-Root" -p $RootPassword
}
